...
# python -m torch.distributed.launch --nnodes=1 --nproc_per_node=1 train.py --cfg ../experiments/exp_test.yaml
# python train.py --cfg ../experiments/exp_test.yaml
...
import dataset
from dataset import make_dataloader, InterHandDataset
from models import AnchorPoseNet, A2JPoseNet, Baseline, HRNetA2JPoseNet
...
def main_worker(gpus, ngpus_per_node, args, final_output_dir, tb_log_dir):
    ...
    dump_input = torch.rand(
        (1, 3, cfg.MODEL.INPUT_SIZE[0], cfg.MODEL.INPUT_SIZE[0])
    )
    
    logger.info(get_model_summary(model, dump_input, verbose=cfg.VERBOSE))
    # input()
    # ops, params = get_model_complexity_info(
    #    model, (3, cfg.MODEL.INPUT_SIZE[0], cfg.MODEL.INPUT_SIZE[0]),
    ...
    # Data loading code
    train_loader = make_dataloader(cfg, is_train=True, distributed=cfg.DISTRIBUTED)
    valid_loader = make_dataloader(cfg, is_train=False, distributed=cfg.DISTRIBUTED)

    logger.info('Training Loader :\n'.format(str(train_loader.dataset)))
    logger.info('Validation Loader :\n'.format(str(valid_loader.dataset)))
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
            ...
            train(cfg, master, train_loader, model, optimizer, epoch,
               writer_dict, logger, fp16=cfg.FP16.ENABLED, device=device)
            ...
            if not cfg.WITHOUT_EVAL:
                logger.info('Start evaluating [{}/{}]'.format(epoch, cfg.TRAIN.END_EPOCH))
                with torch.no_grad():
                    val_total_loss = validate(
                        cfg, args, master, valid_loader, model, 
                        writer_dict, logger, device=device
                    )
            ...
            if master:
                ...
                # generate a lite model for mobile devices
                
                # dummy_input = torch.randn(1, 3, cfg.MODEL.IMAGE_SIZE[0], cfg.MODEL.IMAGE_SIZE[0])
                # scripted_model = torch.jit.trace(model, dummy_input)
                # #scripted_model = torch.jit.script(model)
                # optimized_torchscript_model = optimize_for_mobile(scripted_model)
                # # os.path.splitext(os.path.basename(args.cfg))[0]+".pt"
                # optimized_torchscript_model._save_for_lite_interpreter("RHD.pt")

                # print(optimized_torchscript_model(dummy_input))
...
def save_checkpoint(states, is_best, output_dir,
    ...
    if is_best:
        ...
        torch.save(states['state_dict'], os.path.join(output_dir, 'best_state.pth.tar'))
