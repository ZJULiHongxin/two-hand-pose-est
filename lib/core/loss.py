...
class JointsKLLoss(nn.Module):
    ...
    def forward(self, pose_pred, pose_gt, var, visibility=None):
        # pose_pred/gt: batch_size x N_joints x 2/3
        # var: b x n_joints x 2/3
        # visibility: batch_size x N_joints 
        return (torch.exp(-var) * self.smooth_L1_loss(target=pose_gt, input=pose_pred) + 0.5 * var)
