...
class PoseHighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # input(y_list[0].shape) b x 32 x 64 x 64
        #inter_feat = y_list[0] # used for predRNN
        ...
        # Upsampling
        #         In applications to semantic segmentation and facial landmark
        # detection, we mix the output representations (Figure 3
        # (b)), from all the four resolutions through a 1  1 convolution,
        # and produce a 15C-dimensional representation. Then,
        # we pass the mixed representation at each position to a linear
        # classifier/regressor with t he s oftmax/MSE l oss t o predict
        # the segmentation maps/facial landmark heatmaps
        x0_h, x0_w = x[0].size(2), x[0].size(3)
        x1 = F.upsample(x[1], size=(x0_h, x0_w), mode='bilinear')
        x2 = F.upsample(x[2], size=(x0_h, x0_w), mode='bilinear')
        x3 = F.upsample(x[3], size=(x0_h, x0_w), mode='bilinear')

        inter_feat = torch.cat([x[0], x1, x2, x3], 1)
        x = self.last_layer(inter_feat)
        
        return x, inter_feat # size b x 32 x 64 x 64
